...
public interface MessageResponsitory extends JpaRepository<Message,Integer>,JpaSpecificationExecutor<Message> {
    ...
    @Query(value = "select * from t_message where collections=1 ORDER BY message_id desc",nativeQuery = true)
    List<Message> findAllIndex();

    @Query("select m from Message m where m.title like %?1%")
